...
```python
cube = 8
for guess in range(cube+1):     # can use for loop here as we know that cube root must be less than cube itself
    if guess**3 == cube:
        print('Cube root of', cube, 'is', guess)     # will only print something out if we work with a positive perfect cube
```

```python
cube = 8
for guess in range(abs(cube+1)):
    if guess**3 >= abs(cube):                       # find guess
        break
if guess**3 != abs(cube):                           # guess was greater than the absolute value of the cube
    print(cube, ' is not a perfect cube!')
else:
    if cube < 0:
        guess = -guess                              # flip sign of guess if cube is negative
    print('Cube root of', cube, 'is', guess)        # print guess as solution
```
